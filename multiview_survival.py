        ...
        for experiment in exps:#survival type
            for expindex in range(0,1):
                ...
                for index,split in enumerate(splits[experiment]):                    
                    ...
                    radiomics_tr = robust_scale(radiomics.loc[r_tr_split],unit_variance=True,quantile_range=(10.0, 90.0))
                    radiomics_tr[radiomics_tr>1]=1
                    radiomics_tr[radiomics_tr<-1]=-1
                    radiomics_tr = pd.DataFrame(data=radiomics_tr,index=radiomics.loc[r_tr_split].index, columns=radiomics.loc[r_tr_split].columns)
                    transcriptomics_tr = robust_scale(transcriptomics.loc[r_tr_split],unit_variance=True,quantile_range=(10.0, 90.0))
                    transcriptomics_tr[transcriptomics_tr>1]=1
                    transcriptomics_tr[transcriptomics_tr<-1]=-1
                    transcriptomics_tr = pd.DataFrame(data=transcriptomics_tr,index=transcriptomics.loc[r_tr_split].index, columns=transcriptomics.loc[r_tr_split].columns)
                    deep_tr = robust_scale(deep[model_name].loc[r_tr_split],unit_variance=True,quantile_range=(10.0, 90.0))
                    deep_tr[deep_tr>1]=1
                    deep_tr[deep_tr<-1]=-1
                    
                    #standardize testing set
                    radiomics_tst = robust_scale(radiomics.loc[r_tst_split],unit_variance=True,quantile_range=(10.0, 90.0))
                    radiomics_tst[radiomics_tst>1]=1
                    radiomics_tst[radiomics_tst<-1]=-1
                    radiomics_tst = pd.DataFrame(data=radiomics_tst,index=radiomics.loc[r_tst_split].index, columns=radiomics.loc[r_tst_split].columns)
                    transcriptomics_tst = robust_scale(transcriptomics.loc[r_tst_split],unit_variance=True,quantile_range=(10.0, 90.0))
                    transcriptomics_tst[transcriptomics_tst>1]=1
                    transcriptomics_tst[transcriptomics_tst<-1]=-1
                    transcriptomics_tst = pd.DataFrame(data=transcriptomics_tst,index=transcriptomics.loc[r_tst_split].index, columns=transcriptomics.loc[r_tst_split].columns)
                    deep_tst = robust_scale(deep[model_name].loc[r_tst_split],unit_variance=True,quantile_range=(10.0, 90.0))
                    deep_tst[deep_tst>1]=1
                    deep_tst[deep_tst<-1]=-1
                    
                    columns=["f"+str(s) for s in deep[model_name].columns]
                    deep_tr = pd.DataFrame(data=deep_tr,index=deep[model_name].loc[r_tr_split].index, columns=columns)
                    deep_tst = pd.DataFrame(data=deep_tst,index=deep[model_name].loc[r_tst_split].index, columns=columns)
                    ...
                    try:
                        ...
                        transcriptomics_feat, transcriptomics_coefs = apply_feature_selection(transcriptomics_tr, g_labels, cutoff_pvalue=0.005, c_value=0.6)
                        print("---Radiomics")
                        radiomic_feat, radiomic_coefs = apply_feature_selection(radiomics_tr, r_labels, cutoff_pvalue=0.05, c_value=0.8)
                        print("---Deep")
                        deep_feat, deep_coefs = apply_feature_selection(deep_tr, r_labels, cutoff_pvalue=0.05, c_value=0.6)
                        print("---Clinical")
                    ...
                    selected_transcriptomics = get_features(selected_transcriptomics, r_tr_split, transcriptomics_feat)
                    selected_transcriptomics = get_features(selected_transcriptomics, r_tst_split, transcriptomics_feat)
                    
                    selected_radiomics={}
                    selected_radiomics = get_features(selected_radiomics, r_tr_split, radiomic_feat)
                    selected_radiomics = get_features(selected_radiomics, r_tst_split, radiomic_feat)
                    
                    selected_deep={}
                    selected_deep = get_features(selected_deep, r_tr_split, deep_feat)
                    selected_deep = get_features(selected_deep, r_tst_split, deep_feat)
